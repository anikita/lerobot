#!/usr/bin/env python3
"""
2_train_q_head_v6.py — Train Q-head on pre-extracted features (v6: reviewer fixes).

v6 changes from v5:
  - Fix #1: hidden_dim init guarded (no crash without --dead-zone)
  - Fix #2: dead-zone frames truly excluded from training + metrics (not relabeled)
  - Fix #3: TCN default pool="last" (was "mean", 13.5x less sensitive)
  - Fix #4: BatchNorm1d → LayerNorm in CausalConvBlock (no train/eval mismatch)
  - Fix #7: temporal models error on T<2 (was silent MLP fallback)
  - TCN docstring: 124→125

Loads pre-extracted features and trains a Q-head (MLP, TCN, or CausalTemporalTransformer).
Supports optional VLM adapter fine-tuning (Path C-lite).
...
def load_feature_data(features_dir, feature_field, val_rounds, train_split=0.0,
    ...
    for f in files:
        ...
        # Dead zone: mask frames around intervention onsets
        # v6: dead_mask tracks excluded frames (true exclusion, not just relabeling)
        dead_mask = torch.zeros(len(q), dtype=torch.bool)
        n_dead = 0
        if dead_zone > 0 and interv is not None:
            interv_np = np.array(list(interv))
            onsets = np.where(interv_np[1:] & ~interv_np[:-1])[0] + 1
            for onset in onsets:
                lo = max(0, onset - dead_zone)
                hi = min(len(q), onset + dead_zone)
                dead_mask[lo:hi] = True
            q[dead_mask] = 0.0  # relabel to neutral
            n_dead = dead_mask.sum().item()
            n_onsets = len(onsets)

        # Init hidden_dim from first valid file (fixes #1: crash when dead_zone==0)
        if hidden_dim is None:
            hidden_dim = h.shape[1]
        elif h.shape[1] != hidden_dim:
            print(f"  ⚠️  {f.name}: dim mismatch ({h.shape[1]} vs {hidden_dim}) — skipping")
            continue
        ...
        if is_both:
            ...
            train_dm, val_dm_split = dead_mask[:n_train_frames], dead_mask[n_train_frames:]
            tag = "BOTH "
            ...
            val_rounds_list.append({"name": f.stem + "_val", "h": val_h_split, "q": val_q_split,
                                    "dead_mask": val_dm_split})
        elif is_val:
            tag = "VAL  "
            val_rounds_list.append({"name": key_clean, "h": h, "q": q, "dead_mask": dead_mask})
        elif is_train:
            tag = "TRAIN"
            train_rounds.append({"name": f.stem, "h": h, "q": q, "dead_mask": dead_mask})
        ...
        for tr in train_rounds:
            ...
            if n_holdout > 0:
                train_holdout.append({
                    ...
                    "h": tr["h"][-n_holdout:],
                    "q": tr["q"][-n_holdout:],
                    "dead_mask": tr["dead_mask"][-n_holdout:],
                })
                tr["h"] = tr["h"][:-n_holdout]
                tr["q"] = tr["q"][:-n_holdout]
...
def train(adapter, q_head, train_rounds, val_rounds_list, train_holdout, T,
    ...
    if T == 1:
        # Flatten all training frames, exclude dead-zone frames (v6: true exclusion, fixes #2)
        train_h_list, train_q_list = [], []
        for tr in train_rounds:
            alive = ~tr["dead_mask"]
            train_h_list.append(tr["h"][alive])
            train_q_list.append(tr["q"][alive])
        train_h_flat = torch.cat(train_h_list, dim=0) if train_h_list else torch.empty(0)
        train_q_flat = torch.cat(train_q_list, dim=0) if train_q_list else torch.empty(0)
        if train_h_flat.numel() == 0:
            raise RuntimeError("No non-dead training frames — reduce --dead-zone")
    else:
        # Build window index: (round_idx, last_frame_idx, q_target)
        # v6: skip windows whose target frame is in the dead zone (true exclusion, fixes #2)
        train_windows = []
        n_skipped_dead = 0
        for ri, tr in enumerate(train_rounds):
            ...
            for i in range(T - 1, n):
                if tr["dead_mask"][i]:
                    n_skipped_dead += 1
                    continue  # skip dead-zone target frames
                train_windows.append((ri, i, tr["q"][i].item()))
        print(f"  Training windows: {len(train_windows):,}  (T={T})"
              + (f"  skipped_dead={n_skipped_dead:,}" if n_skipped_dead else ""))

    # ── Build flat val arrays (with dead-zone masks, v6) ──
    if val_rounds_list:
        val_h_flat = torch.cat([vr["h"] for vr in val_rounds_list], dim=0)
        val_q_flat = torch.cat([vr["q"] for vr in val_rounds_list], dim=0)
        val_dead_flat = torch.cat([vr["dead_mask"] for vr in val_rounds_list], dim=0)
    else:
    ...
    # ── Null baseline (exclude dead-zone frames, v6) ──
    val_alive_flat = ~val_dead_flat
    val_neg_mask_bl = (val_q_flat < -0.05) & val_alive_flat
    val_pos_mask_bl = (val_q_flat > 0.05) & val_alive_flat
    null_neg = (val_q_flat[val_neg_mask_bl]**2).mean().item() if val_neg_mask_bl.any() else 0.0
    null_pos = (val_q_flat[val_pos_mask_bl]**2).mean().item() if val_pos_mask_bl.any() else 0.0
    print(f"\nNull baseline — Neg: {null_neg:.4f}  Pos: {null_pos:.4f}"
          + (f"  (excl dead={val_dead_flat.sum().item():.0f} frames)" if val_dead_flat.any() else ""))
            ...
            if T == 1 or is_mlp:
                ...
                for vr in val_rounds_list:
                    ...
                    for i in range(T - 1, n, batch_size):
                        ...
                        val_preds.append(vp)
                val_pred = torch.cat(val_preds)
                # Trim val_q and dead_mask to match (first T-1 frames have no prediction)
                val_q_eval = torch.cat([vr["q"][T-1:] for vr in val_rounds_list])
                val_dead_eval = torch.cat([vr["dead_mask"][T-1:] for vr in val_rounds_list])
                val_alive_eval = ~val_dead_eval
                val_neg_mask = (val_q_eval < -0.05) & val_alive_eval
                val_pos_mask = (val_q_eval > 0.05) & val_alive_eval
                val_neu_mask = ~val_neg_mask & ~val_pos_mask & val_alive_eval

            if T == 1 or is_mlp:
                val_q_eval = val_q_flat
                val_alive_eval = ~val_dead_flat
                val_neg_mask = (val_q_flat < -0.05) & val_alive_eval
                val_pos_mask = (val_q_flat > 0.05) & val_alive_eval
                val_neu_mask = (val_q_flat.abs() <= 0.05) & val_alive_eval
        ...
        if epoch % max(1, epochs // 10) == 0 or epoch == epochs - 1:
            ...
            if train_holdout:
                ...
                with torch.no_grad():
                    ...
                    hneu = (hold_q.abs() <= 0.05) & hold_alive
...
def main():
    ...
    # v6: temporal models require T>=2 (silent MLP fallback was a bug, reviewer #7)
    if model_name in ("causal_tf", "tcn", "transformer") and T < 2:
        print(f"❌ --model {model_name} requires --temporal-window >= 2")
        return
    if model_name == "mlp" or (T == 1 and model_name not in ("causal_tf", "tcn", "transformer")):
